...
- `logs/{suite}/{start}_to_{end}.json` — one per shard: `{category: {total_count, success_count}}`
- `videos/{suite}/rollout_<task_name>_episodeN_<success|failure>.mp4`
- `overall_results.json` — aggregated per-category + per-suite rates and a
  `leaderboard_summary_percent` row matching the LIBERO-plus README table
  columns (Camera / Robot / Language / Light / Background / Noise / Layout /
  Total).

Each suite's category `total_count` summed across shards equals the suite task
count (× `num_trials_per_task`, which defaults to 1): libero_spatial=2402,
libero_object=2518, libero_goal=2591, libero_10=2519.
